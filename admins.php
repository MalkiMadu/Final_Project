...
if(isset($_POST['add_product'])){
    ...
    $p_image_folder = 'snacks_img/'.$p_image;

    $insert_query =mysqli_query($con,"INSERT INTO snack(name,price,image) VALUES('$p_name','$p_price','$p_image')")or die('query failed');
...
if(isset($_GET['delete'])){
    ...
    $delete_query = mysqli_query($con,"DELETE FROM snack WHERE id=$delete_id");
    if($delete_query ){
    ...
    <body>
      <?php 
        if(isset($message)){
            ...
            if($message == $message){
                echo '<div class="message"><span>'.$message.'<i class="fa fa-times-rectangle" 
                onclick="this.parentElement.style.display=`none`;"</i></span> </div>';
        ...
        <div class="container">
            ...
            <section class="display-product-tab">
                ...
                <table>
                    ...
                    <tbody>
                        <?php
                        $select_product=mysqli_query($con,"select * from snack");
                        if(mysqli_num_rows($select_product)>0){
                            while($row= mysqli_fetch_assoc($select_product)){    
                        ?>
                        <tr>
                        <td><img src="snacks_img/<?php echo $row['image']; ?>" height="100" width="200" alt=""></td>
                        <td><?php echo $row['name']; ?></td>
                        <td>Rs. <?php echo $row['price']; ?>.00</td>
                        <td><a href="admins.php?delete=<?php echo $row['id']; ?>" class="delete-btn" onclick="return confirm('Are you sure you want to delete this?');"><i class="fas fa-trash"></i>Delete</a></td>
